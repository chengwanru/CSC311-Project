...
Stage 1  Model hyperparameter selection
           ...
           (Exploratory / appendix only; final model A uses fixed hypers in ../stacking_ensemble.py.)
...
Stage 3  Meta-classifier training
           LogisticRegression (C=META_C, default 1.0) on the 9-dim OOF meta-features.
...
Run from repo root: python appendix_code/stacking_ensemble_multiRF.py
"""
...
K_FOLDS       = 5
META_C        = 1.0   # moderate regularisation — reduces overfitting to seed-specific OOF patterns
N_OOF_REPEATS = 5     # repeat OOF with different fold shuffles and average probabilities
RF_SEED_OFFSETS = [0, 1000, 2000]  # train 3 RFs with different seeds and average
...
def main():
    ...
    print(f"\n[Stage 2] OOF prediction generation on {n_tv}-row trainval pool"
          f" ({N_OOF_REPEATS} repeats × {K_FOLDS} folds, RF averaged over"
          f" {len(RF_SEED_OFFSETS)} seeds)...")

    # Accumulate OOF probabilities across N_OOF_REPEATS independent fold shuffles.
    # Each repeat uses a different shuffle seed so fold-assignment variance averages out.
    acc_lr = np.zeros((n_tv, K))
    acc_nb = np.zeros((n_tv, K))
    acc_rf = np.zeros((n_tv, K))

    tv_ids = df_tv["unique_id"].unique()

    for rep in range(N_OOF_REPEATS):
        rng      = np.random.RandomState(RANDOM_STATE + rep)
        ids_shuf = tv_ids.copy()
        rng.shuffle(ids_shuf)
        id_folds = np.array_split(ids_shuf, K_FOLDS)
        ...
        for fold_i, fold_val_ids in enumerate(id_folds):
            ...
            if rep == 0:
                print(f"  Rep 1  Fold {fold_i+1}/{K_FOLDS}: train={tr_mask.sum()}"
                      f"  val={vl_mask.sum()}")

            # LR (deterministic — one fit per fold)
            lr_f = LogisticRegression(
                max_iter=2500, C=best_lr_C, class_weight=LR_CLASS_WEIGHT,
                solver="lbfgs", random_state=RANDOM_STATE)
            lr_f.fit(X_tv[tr_mask], y_tv[tr_mask])
            acc_lr[vl_mask] += lr_f.predict_proba(X_tv[vl_mask])

            # NB (deterministic — one fit per fold)
            ...
            acc_nb[vl_mask] += _nb_proba(X_nb_tv[vl_mask], nb_f, cnb_f, best_nb_blend)

            # RF — average over RF_SEED_OFFSETS to decouple forest randomness from
            # the data-split seed
            rf_proba = np.zeros((vl_mask.sum(), K))
            for off in RF_SEED_OFFSETS:
                rf_f = RandomForestClassifier(
                    n_estimators=best_rf_nest, max_depth=best_rf_depth,
                    random_state=RANDOM_STATE + off, n_jobs=-1)
                rf_f.fit(X_tv[tr_mask], y_tv[tr_mask])
                rf_proba += rf_f.predict_proba(X_tv[vl_mask])
            acc_rf[vl_mask] += rf_proba / len(RF_SEED_OFFSETS)

    oof_lr = acc_lr / N_OOF_REPEATS
    oof_nb = acc_nb / N_OOF_REPEATS
    oof_rf = acc_rf / N_OOF_REPEATS
    ...
    print(f"\n[Stage 3] Training meta-LR (C={META_C:g}) on 9-dim OOF features...")
    ...
    # RF — average over RF_SEED_OFFSETS (mirrors the OOF treatment above)
    p_rf_test = np.zeros((len(y_test), K))
    for off in RF_SEED_OFFSETS:
