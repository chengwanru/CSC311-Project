...
Reads directly from training_data.csv — the same raw source used by the
LR and RF pipelines — via pipeline.clean() and pipeline.COL_* constants.
Inline parsing with pipeline.extract_likert() and pipeline.extract_numeric()
...
from pipeline import (
    ...
    grouped_kfold_split, regular_split, clean,
    COL_TARGET, COL_ID,
    COL_SOMBRE, COL_CONTENT, COL_CALM, COL_UNEASY,
    COL_EMOTION, COL_N_COLOURS, COL_N_OBJECTS, COL_PRICE,
    COL_ROOM, COL_WHO, COL_SEASON,
    COL_DESC, COL_FOOD, COL_SOUNDTRACK,
    extract_likert, extract_numeric,
)
...
RANDOM_SEED = RANDOM_STATE   # alias kept for backward compatibility
K_FOLDS     = 5
USE_KFOLD   = True
...
# ============================================================
# 1.  COLUMN NAMES  (mapped from pipeline constants)
# ============================================================

TARGET_COL = COL_TARGET   # "Painting"
ID_COL     = COL_ID       # "unique_id"

# Likert survey responses — parsed inline by extract_likert()
LIKERT_COLS = [COL_SOMBRE, COL_CONTENT, COL_CALM, COL_UNEASY]

# Numeric count/rating columns — parsed inline by extract_numeric()
# P99 upper clip fitted on train to prevent outlier leakage.
NUM_COLS           = [COL_EMOTION, COL_N_COLOURS, COL_N_OBJECTS]
PRICE_COL          = COL_PRICE   # parsed + log1p-transformed in build_features
P99_UPPER_NUM_COLS = [COL_N_COLOURS, COL_N_OBJECTS]   # count cols only

# Comma-separated strings → one-hot encoded in build_features
MULTI_COLS  = [COL_ROOM, COL_WHO, COL_SEASON]

# Short keys for storing multi-label vocabularies in fit_info
_MULTI_KEY  = {COL_ROOM: "room", COL_WHO: "companion", COL_SEASON: "season"}

# Free-text columns — concatenated and vectorised
...
def build_features(df, fit_info=None, params=None):
    """
    Transform a raw training_data.csv DataFrame into a feature matrix.

    Accepts the same format produced by pipeline.clean() — long survey-question
    column names, raw Likert text ("1 - Strongly disagree"), raw numeric text,
    and raw price strings.  Parsing is done inline; no preprocessed.csv needed.

    fit_info : dict returned by a previous fit call (pass when transforming
               val/test to avoid leakage).  None → fit statistics from df.
               Stores: vocab, likert_medians, numeric_p99_upper, num_medians,
               bin_edges or num_mean/num_std, multi-label vocabularies.
    ...
    # ── 4b. Likert features ───────────────────────────────────
    # Parse raw survey text (e.g. "1 - Strongly disagree") → integer 1–5.
    # Medians are fitted on training data only to impute missing values.
    if fit_info is None:
        info["likert_medians"] = []
        for c in LIKERT_COLS:
            raw   = extract_likert(df[c])          # 0 where missing/unparseable
            valid = raw[raw > 0]
            info["likert_medians"].append(
                float(np.median(valid)) if len(valid) > 0 else 3.0
            )

    likert_mat = np.column_stack([
        extract_likert(df[c], impute=info["likert_medians"][i])
        for i, c in enumerate(LIKERT_COLS)
    ]).astype(np.float32)
    parts.append(likert_mat)                                   # (n, 4)

    # ── 4c. Numeric + price features ─────────────────────────
    # Parse raw text counts/ratings; apply log1p to price.
    # P99 upper clip and imputation are fitted on training data only.
    if fit_info is None:
        info["numeric_p99_upper"] = {}
        for c in P99_UPPER_NUM_COLS:
            raw    = extract_numeric(df[c])
            finite = raw[raw > 0]
            info["numeric_p99_upper"][c] = (
                float(np.ceil(np.percentile(finite, 99)))
    ...
    for c in NUM_COLS:
        ...
        num_cols_parsed.append(extract_numeric(df[c], clip_max=clip_hi))
    num_cols_parsed.append(np.log1p(extract_numeric(df[PRICE_COL])))  # price

    num_arr = np.column_stack(num_cols_parsed).astype(np.float32)

    if fit_info is None:
        # Median of non-zero (non-missing) values, fitted on train only
        info["num_medians"] = np.array([
            float(np.median(col[col > 0])) if np.any(col > 0) else 0.0
            for col in num_arr.T
        ], dtype=np.float32)

    # Impute zeros (proxy for NaN / parse failures) with train medians
    for j in range(num_arr.shape[1]):
        zero_mask = num_arr[:, j] == 0
        num_arr[zero_mask, j] = float(info["num_medians"][j])

    if params["num_bins"] is not None:
        # Discretise into bins → feed into Multinomial NB
        n_bins   = params["num_bins"]
        n_num_cols = num_arr.shape[1]
        if fit_info is None:
            bin_edges = []
            for j in range(n_num_cols):
                col   = num_arr[:, j]
                edges = np.linspace(col.min(), col.max() + 1e-9, n_bins + 1)
                bin_edges.append(edges)
            info["bin_edges"] = bin_edges

        binned = np.zeros_like(num_arr, dtype=np.float32)
        for j in range(n_num_cols):
            binned[:, j] = (
                np.digitize(num_arr[:, j], info["bin_edges"][j][:-1]) - 1
            )
        parts.append(binned)
    else:
        ...
        if fit_info is None:
            info["num_mean"] = num_arr.mean(axis=0)
            info["num_std"]  = num_arr.std(axis=0) + 1e-9
        num_arr = (num_arr - info["num_mean"]) / info["num_std"]
        parts.append(num_arr)

    # ── 4d. Multi-label categorical features → one-hot ────────
    multi_parts = []
    for col in MULTI_COLS:
        kv_key = f"multi_{_MULTI_KEY[col]}"    # short readable key in fit_info
...
if __name__ == "__main__":
    ...
    print(f"  {len(df)} rows before cleaning")
    df = clean(df)
    print(f"  {len(df)} rows after clean() "
          f"| {df[ID_COL].nunique()} students "
